class GuestReviewsController < ApplicationController
  ...
  def create
    # Step 1: Check if the reservation exist (gym_id, host_id, host_id)
    ...
    @reservation = Reservation.where(
                    ...
                    gym_id: guest_review_params[:gym_id]
                   ).first

    if !@reservation.nil? && @reservation.gym.user.id == guest_review_params[:host_id].to_i
  ...
  private
    def guest_review_params
      params.require(:guest_review).permit(:comment, :star, :gym_id, :reservation_id, :host_id)
    end
end